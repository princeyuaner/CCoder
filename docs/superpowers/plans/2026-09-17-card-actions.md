...
| 计划里写的 | 实际 | 为什么 |
|---|---|---|
| Task 1 给 `Indicator.Meter` 加 `warn` | **没要** | `paintMeter` 本来就是"只有警示色调才覆盖指示器颜色"，压缩中整张卡是 Warn，条子自己会转琥珀 |
| Task 2 = 值行换画法（方案甲） | 改成**右上角常驻图标**（方案乙） | 真机上悬停被子件的 tooltip 偷走，用户当场否掉（见上面那条改版记录） |
| Task 3（抽 `sendText`）单独提交 | **并进 Task 4** 那一笔 | 两者改的是同一个文件（`ClaudePanel.kt`）的相邻位置，拆开要动 hunk；抽完的中间态没有别的入口，单独提交的收益只有"回滚到抽了一半"，不值当 |
| 图形按设计稿定 | 三轮试错后才**出候选表让用户挑** | "斜杠""纸飞机""带柄小球"都读不出来——14px 上细描边围出的形状会塌。猜三轮不如列 12 个 |

**最终提交**（分支 `v0.2.18-dev`）：docs（设计稿+spec+plan）/ docs（rewind 留底）/
chore(sidecar)（探针）/ 纯逻辑 / 视图 / 接线 / fix(ui)（链接对比度）/ What's New。

### 渲染出来才发现的错

- **压缩中那张图里，连接卡的清空按钮还亮着** —— 第一版忘给连接卡传 `busy`。
  压缩是一个回合，会话是忙的，现实里不存在"压缩中而清空还亮着"这种组合。
  这条只有画出来才看得见（单测里两张卡是分开测的）
- **「等待响应」标签与右上角图标之间只剩 3px**（`status-cards-probe-actions-waiting.png`
  量的）。不重叠，但紧。**用户尚未拍**：缩短成「等待中」/ 给标签行让出角上的位置 /
  就这样 —— 当前按"不动文案、不动布局"落的
- 同一文件里现在有**两套 LAF**：旧的 `render(...)` 还在 Metal 下画（2026-09-14 写的，
  没跟上 09-15 那条规矩），新加的 `renderActions` 用 `IdeLaf.withRealLaf`。旧图浅色、
  新图深色。迁移建议单独做一次（会改掉已提交的图，与本版无关）

### 仍未做的（都是"要有真机才能做"的）

- **手工冒烟清单**（见上）：点按手感、忙时灰态、压缩期间转写区、回执与百分比谁先、
  tooltip 的判定、启动失败下的灰态。自动化测不到 `ClaudePanel` 的接线
- **真机观感没验过**：自动化只到 `buildPlugin` + `verifyPlugin` 为止；装进 IDE 的
  观感（就是上面那份冒烟清单）要自己走一遍才算数 —— 悬停那版正是"全绿但用不了"
