...
> **规则简化成一条更好记的：lambda 类型的参数永远放最后。**
>
> （补记 2026-09-13：做「会话列表居中」时在 `showTogglePopup` 上**又犯了一次** ——
> 把 `centerOverPanel: Boolean` 加在 `onClosed: () -> Unit` 后面，三个调用点的
> 尾随 lambda 立刻改绑。这次编译器逮住了，因为**两边类型不同**
> （`Function0` vs `Boolean`）。上一次 `onPick`/`onDelete` 两边都是 `Function1`，
> 类型一致，编译器一声不吭，一路红到手工冒烟。
> **能不能被编译器拦住纯属运气 —— 所以别指望它，按规则写。**）
>
> ---
>
> ### 冒烟后又修掉两条（2026-09-13）
>
> 手工冒烟反馈：「点击历史会话没反应」「删除按钮有是有但是看不清楚」。
> 两条都是**既有测试结构上看不见的**：
>
> **① 点会话没反应 —— 事件被吃掉了。**
>
> 为了让 ✕ 悬停时不闪，我给标题/时间等子组件也挂了鼠标监听器。但真鼠标点击时
> 事件发给**鼠标底下最深的有监听器的组件**，不是你想的那个容器 —— 点行中间正好
> 落在标题上，事件被标题吃掉，行收不到。
>
> **既有用例全都是 `component.dispatchEvent(...)` 直接往"行"上派发**，绕过了
> AWT 的真实命中路径。17 条全绿，真的一点用都没派上。
>
> 修法：同一个 `MouseAdapter` 挂到行**和它的每个子组件**上。两边都得挂 ——
> 只挂行则点标题被吃掉，只挂子组件则点行内空白没人接。
>
> **这条教训是通用的，也是本次最值钱的一条：**
> **在 Swing 里测"点某处会发生什么"，`dispatchEvent` 到目标组件测不出真行为。**
> 要测就得往**鼠标实际会落在的那个子组件**上派发。补的复现用例
> 「点标题也算点这一行」就是这么写的 —— 先写它、看它红、再修。
>
> **② 删除按钮看不清 —— 离屏渲染一看就明白。**
>
> 那是个淡灰的小 ✕，用正文的**次要色**（`getInactiveTextColor`）画的，
> 在弹出层底色上几乎融进去。改成比正文大一号 + 正常前景色，并补上设计稿
> §二 A 本来就画过的**悬停变红**。
>
> 这条单测同样测不出（属性都对），**是画出来看的**。见
> `build/session-list-probe-hover.png`（改前改后各看过一次）。
