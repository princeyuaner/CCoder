...
> **2026-09-15 改：三栏改成四个真页签，「设置」从此只有这一个入口。**
> 原方案是「左栏 132px 假页签 + 模型列表 + 编辑表单」，左栏是给「通用 / 权限 / 关于」
> 留的位置 —— 而那次只做了模型页（见下面最后那条"本版只做模型一页"），
> 于是它既不导航也不装饰，白占六分之一块屏幕（`docs/design/settings-v2.html` 上有真机图）。
> 现方案 **C**：左栏兑现成 **模型 / 通用 / 权限 / 环境** 四个真页签，
> 后三页就是 IDE 那页 `Settings → Tools → CCoder` 的六项换个地方摆，
> **那个 `<projectConfigurable>` 连同 `ClaudeSettingsPanel.kt` 一并删掉**。
> 尺寸 860×540 → **860×560**；底部换成一**颗**「关闭」（原来的「取消」在这页上
> 什么都不回滚，代码注释自己承认过）。
>
> **保存语义统一成"即时保存"**（原来是"IDE 那页攒到 apply"，两套并存）。
> **「权限模式」「思考深度」改完立刻作用到正在跑的会话** —— 判定在
> `ui/SettingsApply.kt`，`ClaudePanel` 在关框后对账。
>
> 下图的布局逐条仍然成立（字段顺序、删除的位置、密钥打码），只是外框换了。

`DialogWrapper`，860×560，左侧竖排页签（140px）。

```
┌─ 设置 ──────────────────────────────────────────────── ✕ ─┐
│ ┌────────┬──────────────┬──────────────────────────────┐ │
│ │ 模型    │ ● DeepSeek   │ 名称      [DeepSeek        ] │ │
│ │ 通用    │   官方 Sonnet │ Base URL  [https://api.d…  ] │ │
│ │ 权限    │   中转 Opus   │ 认证方式  [Bearer ▾        ] │ │
│ │ 环境    │              │ API Key   [sk-••••••••  👁 ] │ │
│ │        │ ＋ 添加配置    │ 模型 ID   [deepseek-flash… ] │ │
│ └────────┴──────────────┴──────────────────────────────┘ │
│                                       改动即时保存 [关闭]  │
└──────────────────────────────────────────────────────────┘
```
...
- 「删除」放在表单**底部左**，与「关闭」分开 —— 它和"保存这次编辑"不是一类动作
- 密钥字段默认打码，👁 切换明文；失焦即恢复打码
- **列表行的背景高亮 = 我正在改哪条**，右侧「使用中」= **哪条在生效**。
  两个信号回答两个问题，别并成一个（`ModelProfilesPage.rebuildList` 的注释）
- ~~本版只做「模型」一页，其余三页签先不放~~ —— **2026-09-15 已补齐**（上面那段）
...
| `settings/ModelProfilesDialog.kt` | 设置对话框（`DialogWrapper`）。**2026-09-15 拆成** `SettingsDialog`（外壳）+ `ModelProfilesPage`（模型页）+ 三个页类 |
...
- ~~**其余三个页签**（通用/权限/关于）—— 本版只做模型页，页签位置留好~~
  **2026-09-15 补上了**，但页签是「通用 / 权限 / 环境」，而且不再留位置 —— 直接做了（§7 顶上那段）
