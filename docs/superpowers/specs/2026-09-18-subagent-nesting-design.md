...
**折叠规则**：工具卡一律"默认收着"（2026-09-14 用户定的），但**带子代理那块的卡破一次例
—— 它第一次冒出子项时自动展开一次**，之后归用户管（他收起来就一直是收着的，后面再来多少条
也不弹开）。为什么破例：今天（A0）子代理的工具卡是平铺在主流水里、**看得见**的，
A1 把它们收进卡片之后若还收着，就等于"改完看不见了" —— 那不是嵌套，是藏起来。
**跑完不自动收**：收起来会在你正读子代理最后那句话时把它抽走。
