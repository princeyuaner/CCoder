...
**代价**：界面上永远挂着两个「空闲」。（上下文卡后来不在此列 —— 它的词换成了 `0`，见 §6 的修正。）
...
| 上下文 | CLI 的 `getContextUsage()`（见下） | **不适用** —— 没有测量值时显示 `0` |
| 子任务 | `RunStatusTracker.todos` | 从没收到过任务清单（见 §6.1） |
| 子代理 | `RunStatusTracker.running` | 没有在跑的任务 |

**2026-09-14 修正：上下文卡不用「空闲」。** 那个词的意思是"没在跑"（子任务、子代理用它是对的），而上下文恰恰不是这回事 —— 恢复一场长对话之后它一点都不空闲，我们只是**没测过**。按连接卡那条同源的规矩（"未连接是一种状态，不是没数据"），这里把"没数据"说成了一个状态，只是方向相反。现在没有测量值时显示 `0` —— 至少是个能被纠正的数字。

`ContextUsage` 缺窗口容量时（`windowTokens <= 0`）**不给副值**，值上给绝对数即可。原来写"副值只显示已用量"，但那样会拼出 `0 / 0`、`325.4k / 325.4k` 这种把同一个数说两遍的样子。

**两个数都来自 CLI**：`Query.getContextUsage()`（`detail: 'summary'`）—— `/context` 用的就是它。这是 2026-09-14 实测后改的，之前插件自己从 result 事件拼，改的原因有两条：

- **`modelUsage` 是跨回合累计的总额**（`sdk.d.ts:5054`，"Cumulative across turns… each result carries the running total so far"），拿它当"当前占用"会每回合往上涨；
- **恢复会话拿不到窗口**。历史里只有 `usage`、**没有 `contextWindow`**（在真实会话记录里核对过，0 处），SDK 里也只有 `modelUsage` 带它，而它不落历史 —— 于是恢复之后卡片只有绝对数、没有分母。

换成 `getContextUsage()` 之后这两条同时消失。实测（CLI 2.1.268）：

| 事实 | 值 |
|---|---|
| 一条消息都没发就能调吗 | **能**（`query()` 建好即可，`init` 都不用等） |
| 恢复的历史算不算进去 | **算**（resume 一条长会话报 `Messages: 455407`，不是 0） |
| 分母 | `rawMaxTokens`（文档说是 usage "measured against" 的那个窗口） |
| 字段名 | **驼峰**（`totalTokens` / `rawMaxTokens`），不是 d.ts 里写的 snake_case —— 照 d.ts 读会拿到 `undefined` |
| 卡片自己算的百分比 | 与 CLI 报的一致（456990/1000000 → 46%，两边都是 46） |

所以 result 事件的 `usage` / `modelUsage` **一个都不再读**，"按会话记窗口"那套也一并删了。插件侧只负责问和显示。
