...
> **2026-09-15 追加：进转写区的是"用户敲的那份"，不是展开后的那份。**
> 用户提「选中的代码块发送到输出区后显示的格式应该和输入框中的一样」——
> 输入框里是**一行记号**（`⟦路径 24-27 · 4 行⟧`），而此前推给转写区的是
> `snippetRefs.expand` 之后的那份（路径 + 围栏 + 代码全文）：一段几百行的代码
> 铺进气泡，自己写的那句话就淹在里头了。现在两个 push 点（直接发 / 出队发）
> 都用 `typed`；**发给模型的那份（`text`）一个字节都没变**。
> web 侧把 `⟦…⟧` 画上底色，用的是输入框那一对颜色（`--ref-bg` + `--text`）。
>
> 一条已知的不对称：**恢复历史会话时看到的是展开过的那份** —— CLI 存的就是
> 发出去的那份，记号从没离开过输入框。要不要在恢复时把展开的那份再折回记号，
> 是一次单独的判断（只能靠模式猜，猜错会把用户自己贴的代码藏起来），本版不做。
