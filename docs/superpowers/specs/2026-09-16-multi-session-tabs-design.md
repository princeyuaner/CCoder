...
### 4.1 返工：平台那行**关不掉**，所以改成"平台只有一个 content"（2026-09-16 当天）

第一版是"每个会话一个 `Content`，让平台管标签"。真机上出现了**两行**：
平台那行 `TABBED` 与我们自己那排胶囊各画一遍（用户截图来问"为什么有两行页签"）。

试过只关 ✕：`canCloseContents` **只管那颗叉，不管这一行在不在**。
再查 `setContentUiType` / `ToolWindowContentUiType` —— 它只有 `TABBED` 与 `COMBO`
两个值，**没有"不画"**；只要 `ContentManager` 里内容数 > 1，平台一定会画一行。

于是改成：**平台那边永远只有一个 content**，它的组件是个宿主面板，当前那条会话
挂在里面（`removeAll` + `add`，与设置对话框 `pageHost` 同一个做法）。
代价是自己管三件事：当前是哪条、关闭时的确认、销毁时把每条收干净 ——
都在 `SessionTabs` 里。顺带少了两条平台路径（`Close All` / `Ctrl+F4`），
因为现在根本没有平台标签。

**这条要记住的是**：工具窗口的标签**不是我们能决定画不画的** —— 要么用它，
要么把内容数压到 1。
...
- **只有一个标签时**平台会不会多画一条标题栏（`SessionTabs` 给每个标签都设了
  displayName）—— 若显得多余，改成"只有两个以上标签时才给标题"
- 从第二个标签开始，标题会不会与面板自己那一行会话标签重复（两处都在说同一条会话）
- 忙时关标签的确认框：确认与取消两条分支
