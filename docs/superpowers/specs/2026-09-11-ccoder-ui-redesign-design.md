...
├── 头部：状态指示                                    原生
└── JBSplitter (vertical, 可拖动)                     ← 高度可调
    ├── ClaudeTranscriptView (JBCefBrowser)          ← 本次替换的块
    └── 底部区域
        ├── PermissionSlot                           原生，复用 PermissionCard
        └── 输入区
            ├── 输入框（随内容长高，上限 12 行后滚动）   原生
            └── 工具栏：[⋯ 预留 ⋯]        [发送/停止]
```

**为什么输入区改成"输入框在上、工具栏在下"**（2026-09-11 重设计）：原来发送按钮摆在输入框右边，那一行宽度只够放一个按钮。后面要加的模型切换、权限模式、用量读数都无处可放，加一个就要重构一次布局。工具栏独占一行之后，新控件只是往它左侧添，结构不用再动。

**输入框高度：手动，不自动**（2026-09-11 用户明确要求）。默认高度由分隔条比例决定（转写区 0.89 / 底部 0.11），最小行数 1。

- **不做自动长高**：那样"我写多长"会直接顶开面板。手动调把两件事解耦
- 拖高必须**真的**让输入框变大，为此有三处配合缺一不可：
  - `bottom` 里输入区放 `CENTER` 而非 `SOUTH` —— `BorderLayout` 只给 SOUTH 首选高度，多出来的高度会落到空着的 CENTER，拖了等于没拖
  - 输入框用 `ComposerTextArea`，其 `getScrollableTracksViewportHeight()` 返回 true —— `JTextArea` 默认不随视口长高，放在滚动面板里时拖高只会多出一片空白
  - 内容超过可用高度时由滚动面板接管，不需要另外设上限

**发送快捷键可配置**（`SendShortcut`）：聊天惯例 `Enter 发送 / Shift+Enter 换行`（默认）或编辑器惯例 `Enter 换行 / Ctrl+Enter 发送`。两种习惯差异太大，不替用户选。`Shift+Enter` 在两种惯例下都留给换行。

**上下文长度条**（2026-09-11 加）：输入框上方一行，显示 `12.3k / 200k · 6%` —— 已用 token / 上下文窗口 / 占用比例。

- 数据来自 result 事件的 `modelUsage`（`sdk.d.ts:5056`），SDK 直接给出 `contextWindow`，所以比例是真算出来的
- `modelUsage` 是 `Record<模型名, ModelUsage>`，子 agent 与辅助模型各占一项。取**输入量最大**的一项作为上下文占用 —— 主对话的输入包含完整历史，必然最大
- 取不到就不显示这条（初始隐藏），**不造零值**：显示"上下文 0"是假信息
- `contextWindow` 缺失或为 0 时只显示已用量，不做除法
- sidecar 无需改动：它本来就原样转发事件

**当前的上下文行只有"用量"这一种**。规划中的当前文件 / 手动附加 / 选区标签尚未实现，它们同样插在这一行。

对应测试：`ComposerRulesTest`（撑满视口、最小行数、两种快捷键惯例、三段布局位置）、`ContextUsageTest`（取数、取最大、缺失返回 null、缩写与比例格式）。
...
|---|---|---|
| 对齐 | 右 | 左 |
| 最大宽度 | 85% | ~~92%~~ → **不设上限（铺满整行）** |
| 圆角 | 12px | 12px |
| 内边距 | 10px 14px | 10px 14px |
| 背景 | 主色调低饱和变体 | `--surface` |

> **2026-09-14 修订**：Claude 侧（正文与报错）**去掉宽度上限，铺满整行**。92% 只作用在气泡上，而工具卡片、思考块、系统提示这些 `.transcript` 的**直接子项**是天然满宽的 —— 两种排版并排出现就成了"正文没对齐"（用户报「文本输出不是整个聊天框的宽度，工具调用是正常的」；无头 Chromium 实测右边空 8% 行宽：420px 下 38.1px，700px 下 52.8px，随宽度按比例涨）。用户气泡保留 85% —— 右对齐 + 窄气泡是"谁在说话"的分栏手段，助手侧铺满之后反而更清楚。见 `web/src/styles.css` 的 `.bubble--assistant` 与探针的同名断言。
...
**正文里的 Markdown 支持面**（2026-09-14 补）：段落、标题、有序 / 无序列表（含嵌套）、任务列表（`[x]` 画成勾选框）、引用、围栏代码（走 CodeBlock）、**表格**（GFM；超宽时表格自己横向滚动，不撑破气泡）、行内代码、链接、粗斜体、分隔线。

没列的构造一律走**兜底：按纯文本渲染**（不是丢弃）—— 裸 HTML 正是靠这条路挡住的。

> 这次补的原因：表格、嵌套列表、任务列表三类 token 原先**都没有分支**，落到兜底分支把 `token.raw` 原样吐出来，界面上出现的是原始 Markdown（`| 场景 | 改前 |`、`  - 嵌套一层`）。实测本机最近 6 个会话的 188 条正文里有 **25 条含表格**（13%），所以一眼看过去"很乱"。清单见 `web/src/components/Markdown.test.tsx` 的表格 / 嵌套列表 / 任务列表用例。
...
> **2026-09-15 追加：发送消息也算一次"恢复"。** 用户报「发送消息时，如果输出区不在最底部，应该自动滚动到最底部」—— 暂停态下按发送，屏幕上什么都不动，看起来像没发出去。
>
> 判据落在 **"最后一条用户消息的 id 变了"**。发送发生在 Kotlin 侧（输入框是 Swing 的），web 能看见的信号只有那一条 `append(user)`：敲下发送的那一刻就到了（`sendCurrentInput` 里那句 `pushItem(RenderItem.UserText(…))`，不等模型回答）；忙时排队的那条在真正发出时由 `flushQueue` 推 —— 两者都是"发出去了"，所以都算。用 id 而不是"用户消息条数"：`reset` 与回放会把 `items` 整个换掉，条数会在换会话时撞出假阳性。
>
> 已知后果一条：**回放历史（切会话 / 恢复）走的是同一条路**，于是同样落在最底端。那正是"刚换了会话，想看看聊到哪了"该在的位置，所以照收 —— 不是漏网，是有意收下的（单测里专门钉了一条）。
>
> 这一条**没有推翻下面那条"明确不做"**：助手继续输出时视口仍然不动，只有"自己发的"才把视口拽回底部。
...
| 暂停的判据是**滚轮意图**（2026-09-21） | 位置读数看不出用户上滚 —— 见下面那条修订 |
| 我们自己的写入不算用户动作（2026-09-21） | 写入会派发一条 scroll 事件（位置=刚写下的底部）。它到达时**只否决"恢复"**，不否决"暂停" |
...
> **2026-09-21 修订：暂停的判据从"位置读数"换成"滚轮意图"。**
>
> 用户报「思考输出的时候，转写区往上滚动的时候，会自动弹回来」。根因**不是**渲染慢：跟随是每帧一次 `scrollTop = scrollHeight` 的**绝对写入**，而它总排在**同一帧的 scroll steps 之前**（真实浏览器实测帧间隔 16.7ms、这笔写入 0.1ms，见 `2026-09-14-streaming-perf-design.md` §6）—— 于是 `handleScroll` 读到的位置永远是"已经被我们自己写回去"的那一个，**用户上滚这件事从位置读数里根本看不出来**，每一拍都会被拽回。
>
> 于是：
> - **暂停**由 `wheel` 事件当场登记（`deltaY < 0` 即上滚；`deltaY > 0` 且已在底部则恢复）。事件自带方向，不依赖位置读数，也就不会被我们自己的写入污染。两个守卫：`scrollTop > 0`（贴着顶再上滚不暂停；触控板的斜向手势也落在这里），以及把 `programmaticRef` 一并作废（用户接管了平滑动画 —— 留它卡着会让之后**所有** scroll 事件被吞掉，跟随再也关不掉）。**暂停的进入条件因此放宽**：从"离开底部超过阈值"变成"上滚（滚轮）**或**离开阈值" —— 意图优先于位置。
> - **恢复**照旧以位置为准（滚回距底 ≤ 阈值），但加一层**回声盾**：每写一笔置起、被读到一次即销。它**只否决"恢复"**，不否决"暂停"，且此后任何一条真实位移都会重新判一次（自愈，没有时间窗）。
> - **位置读数仍是后备**：拖滚动条、键盘滚动这些没有 `wheel` 事件的情形照旧生效。
>
> 已知代价两条（都写进了用例与探针）：① 上滚那一格若正好落在"推送任务进行中"，位置会被那一拍吃掉一格 —— **之后立刻停得住**，契约要的是这个，不是"一格都不丢"；② 拖滚动条回到底部时，若位置恰好等于我们上次写入的值，恢复会晚一条事件（滚轮向下、点按钮都不受影响）。**明确不做**：键盘/触摸的手势信号（同一个处理器里几行可加，遇到再说）、按 `deltaY` 反推位移的补偿（deltaMode 与平滑滚动下不可靠）。

**「回到底部」按钮**：**只要离开底部就显示**（2026-09-15 改：原来还要求"已有新内容到达"，于是"答完之后往上翻旧内容"时按钮不在 —— 而那正是最想点它的时候，用户报的就是这个）。暂停期间又来了新内容时，按钮上多一个小圆点。浮于转写区右下角，点击 → `behavior: 'smooth'` 滚到底并恢复跟随。

对应测试见 §8.1，手工验证见 §8.4 第 9 条与第 10 条（后者专门盯这次修订）。
...
- **滚动跟随（§4.5）**：内容增长时视口被带到最底端；用户上滚离开底部后新内容不再移动视口；用户滚回底部附近后跟随自动恢复；离开底部后「回到底部」按钮出现（有新内容时带小圆点），点击后恢复跟随；**自己发出一条时无论停在哪里都回到最底端**（2026-09-15 追加，含回放历史的后果一条）；**滚轮上滚立刻暂停（人就停在底部也照样暂停）、贴着顶不暂停、暂停后滚轮向下到底恢复、平滑动画被滚轮接管，以及"我们自己那笔写入的回声不能把暂停撤销"**（2026-09-21 追加，见 §4.5 的修订）
...
10. **流式期间上滚（2026-09-21 修订，必须手工）**：让思考或正文持续流着，(i) 滚轮上滚一次 —— 可能被吃掉一格，接着再滚一下必须停得住，之后不再被拽回；(ii) 停在上面继续等流式，视口一动不动、按钮带小圆点；(iii) 点「回到底部」后立刻再上滚一次，跟随仍能关掉（盯 `programmaticRef` 卡住那条）。**`npm run probe:scroll` 覆盖不了这一条**：它跑的是同一版 Chromium，但没有 JCEF 的输入链路（窗口是否 OSR 还取决于 IDE 设置），`WheelEvent` 也只是个方向信号 —— 合成器那一半只有人眼看得见
