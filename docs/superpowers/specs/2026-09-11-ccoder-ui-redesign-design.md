...
**输入框高度：手动，不自动**（2026-09-11 用户明确要求）。默认 3 行，高度由用户拖分隔条决定。
...
**上下文行暂不实现**：用户规划中的上下文显示（当前文件 / 手动附加 / 选区 / 用量）此刻没有数据源，建一个恒为空的行是白做。它将来作为一行插在输入框上方，不会推翻本次结构。

对应测试：`ComposerRulesTest`（高度上下限、两种快捷键惯例、工具栏必须在下方）。
