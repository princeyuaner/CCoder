...
> **2026-09-16 再补：那 2 条不用猜了；取数来源也换了。**
>
> 1. **命名空间是读出来的，不是猜的。** 把 31 条的描述抬头**全量**打了一遍
>    （`probe-init-before-send.mjs` 末尾那段）：**只有 3 条以 `(x)` 开头，
>    而这 3 条恰好就是要命名空间的那 3 条** —— 其中 `code-review:code-review`
>    自己就带着 `code-review:`，等于直接演示了"括号里那个词就是命名空间"。
>    于是 `sendable` 为空时：显示名自带 `:` 的原样用；描述有前缀就拼
>    `前缀:归一化名`；没有才是裸名。两条端到端都对上（`(frontend-design)` +
>    `frontend-design` → `frontend-design:frontend-design`），并且**前缀是插件名、
>    不一定是技能名**（`(superpowers)` + `brainstorming` → `superpowers:brainstorming`）
>    —— 所以只能读、不能从显示名推。
>    判据只认「开头的 + 形状像 slug 的」括号：用户技能的 `(user)` 在**末尾**，
>    不会被误认。
> 2. **显示那份改问 `initializationResult()`。** 它和 `supportedCommands()`
>    给的是**同一份数据**（31 条，字段都是 `name/description/argumentHint`），
>    但冷启动差 4 倍：**680ms vs 2976ms**（两条各量一次冷启动的对照，见探针注释）。
>    用户看到的差别就是打 `/` 之后列表立刻出来、还是干等三秒。取不到再退回原路。

> ⚠️ 上面第 3 条那句"不解析描述里的 `(superpowers)` / `(user)` 后缀——那是文本，不是结构"
> **只管分组**（分组看的是可发送名里有没有 `namespace:`）。2026-09-16 起
> **取插入值时确实会读描述开头的那个括号** —— 它不是"解析文本猜语义"，
> 是那段文本里唯一的命名空间来源，且 31 条全量核过不误伤。
